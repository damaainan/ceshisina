...
23 Dec 2016[Redis][0][Server][1][Database][2][NoSQL][3]
<font face=微软雅黑>
...
对于 RESP 协议的详细介绍可以看官方文档中的 [Redis Protocol specification][5]，在这里对这个协议进行简单的介绍。

在将不同的数据类型序列化时，会使用第一个 byte 来表示当前数据的数据类型，以便在客户端或服务器在处理时能恢复原来的数据格式。

![redis-resp-data-byte][6]

举一个简单的例子，字符串 OK 以及错误Error Message 等不同种类的信息的 RESP 表示如下：

![redis-resp-type-and-examples][7]
