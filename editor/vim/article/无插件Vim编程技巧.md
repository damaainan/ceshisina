...
相信大家看过《[简明Vim教程][4]》也玩了《[Vim大冒险][5]》的游戏了，相信大家对Vim都有一个好的入门了。我在这里把我日常用Vim编程的一些技巧列出来给大家看看，希望对大家有用，另外，也是一个抛砖引玉的过程，也希望大家把你们的技巧跟贴一下，我会更新到这篇文章中。另外，这篇文章里的这些技巧全都是vim原生态的，不需要你安装什么插件。**我的Vim的版本是7.2**。
...
你可以看到Vim打开了四个文件，编号是4，5，6，7，如果你要切换打开的文件，这个时候，你不要按回车（按了也没事，只不过按了就看不到**`:ls`**输出的**`buffer`**列表了），你可以使用下面的命令切换文件（`buffer`后面的4表示切到4号文件也就是`src/http/ngx_http.c`）：
...
相信你在《[Vim的窗口分屏][8]》一文中，你已经知道了怎么拆分窗口了。其实，我更多的不是用拆分窗口的命令，而是用浏览文件的命令来分隔窗口。如：
...
在分屏间的跳转和切换在《[Vim的窗口分屏][8]》一文中提过了：**先按Ctrl + W，然后按方向键：h j k l**
...
如果文件重复，vim默认会报错，如果你想强行写入的话，你可以在**`mksession`**后加**`!`** ：
...
你可以使用像浏览文件那样用j, k在quckfix窗屏中上下移动到相应的错误上然后按回车，然后就可以在上面的窗屏里定位到相应的源文件的代码行。但是，如果是这样的话， 你要定位下一条错误还得用**`Ctrl +W `**回到quickfix屏中来然后重复来过。
...
比如我们正在浏览nginx的代码，这时，我想看看哪里用到了`nginx`的`NGX_HTTP_VAR_INDEXED`宏。于是，我可以在vim里输入如下的命令：
...
[0]: http://coolshell.cn/articles/11312.html
[1]: http://coolshell.cn/articles/author/haoel
[2]: http://coolshell.cn/articles/11312.html#comments
[4]: http://coolshell.cn/articles/5426.html
[5]: http://coolshell.cn/articles/7166.html
[6]: http://coolshell.cn//wp-content/uploads/2014/03/Explorer.png
[7]: http://coolshell.cn//wp-content/uploads/2014/03/buffer_ls.png
[8]: http://coolshell.cn/articles/1679.html
[9]: http://coolshell.cn//wp-content/uploads/2014/03/WindowsExplorer.png
[10]: http://coolshell.cn//wp-content/uploads/2014/03/TabExplorer.png
[11]: http://coolshell.cn//wp-content/uploads/2014/03/Tab01.png
[12]: http://coolshell.cn//wp-content/uploads/2014/03/quickfix.png
[13]: http://coolshell.cn//wp-content/uploads/2014/03/quickfix_grep.png
[14]: http://coolshell.cn//wp-content/uploads/2014/03/auto_complete_ctrl_n.png
[15]: http://coolshell.cn//wp-content/uploads/2014/03/auto_complete_ctrl_p.png