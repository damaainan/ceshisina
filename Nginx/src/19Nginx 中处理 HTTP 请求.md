...
函数 ngx_http_header_filter 在文件[src/http/ngx_http_header_filter_module.c][4] 中定义如下：
...
HTTP 响应包体由函数 ngx_http_output_filter 发送，该发送函数的执行过程中会遍历过滤链表，该过滤链表的过滤模块是那些对 HTTP 响应包体感兴趣的过滤模块组成，ngx_http_output_filter 函数按照过滤链表的顺序依次处理响应包体，直到最后一个链表元素处理响应包体并把该响应包体发送给客户端。ngx_http_output_filter 函数定义在文件 [src/http/ngx_http_core_module.c][2] 中如下：
```c
    /* 发送HTTP 响应包体 */
    ngx_int_t
...
函数 ngx_http_write_filter 在文件 [src/http/ngx_http_write_filter_module.c][5] 中定义如下：
```c
    /* 发送响应报文数据 */
...
函数 ngx_http_writer 在文件 [src/http/ngx_http_request.c][0] 中定义如下：
```c
    static void
...
当一个动作结束时，会根据引用计数判断是否结束其处理的请求，以下是有关关闭请求的函数；以下函数均在在文件 [src/http/ngx_http_request.c][0] 中定义如下；
...
[0]: http://lxr.nginx.org/source/src/http/ngx_http_request.c
[1]: http://www.nginxguts.com/2011/01/phases/
[2]: http://lxr.nginx.org/source/src/http/ngx_http_core_module.c
[3]: http://lxr.nginx.org/source/src/http/ngx_http_request_body.c
[4]: http://lxr.nginx.org/source/src/http/ngx_http_header_filter_module.c
[5]: http://lxr.nginx.org/source/src/http/ngx_http_write_filter_module.c