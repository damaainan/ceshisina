...
Nginx 的启动初始化由 main 函数完成，该函数是整个 Nginx 的入口，该函数完成 Nginx 启动初始化任务，也是所有功能模块的入口。Nginx 的初始化工作主要是一个类型为 ngx_cycle_t 类型的全局变量。main 函数定义在文件：[src/​core/​nginx.c](http://lxr.nginx.org/source/src/core/nginx.c)
...
在初始化过程中，ngx_cycle_t 类型的全局变量最为重要，该类型结构定义如下：[src/​core/​ngx_cycle.h](http://lxr.nginx.org/source/src/core/ngx_cycle.h)
...
该结构的初始化是通过函数  ngx_init_cycle() 完成的，该函数定义如下：[src/​core/​ngx_cycle.c](http://lxr.nginx.org/source/src/core/ngx_cycle.c)
