...
function al_merge($arrA, $arrB)
{
    ...
    while (count($arrA) && count($arrB)) {
        ...
        $arrC[] = $arrA['0'] < $arrB['0'] ? array_shift($arrA) : array_shift($arrB);  // 数字键名 array_shift 后 从零开始  
    }
...
function al_merge_sort($arr)
{
    ...
    $left_arr = array_slice($arr, 0, $mid);//拆分数组0-mid这部分给左边left_arr    带 offset 的 [ 0,$mid) ，不包括右边
    $right_arr = array_slice($arr, $mid);//拆分数组mid-末尾这部分给右边right_arr   带 offset 的 [$mid ,end ] ,到结尾
    ...
    function mergeSort(array $numbers=array()) 
    {
        ...
        // $half  = ($count >> 1) + ($count & 1);  // 优化  分割成 $half 块 
        ...
        while (count($left) && count($right))// 合并
        {
            if ($left[0] < $right[0])
