...
function dealSQL($arr){
	...
	$stmt2=$pdo->prepare("INSERT INTO weibo (author,createTime,content,isOriginal,rAuthor,rCreateTime,rContent) VALUES(?,?,?,?,?,?,?) ;");
	for($i=0,$len=count($arr);$i<$len;$i++){
	    $stmt2->bindParam(1,$arr[$i]['author']);
	    $stmt2->bindParam(2,$arr[$i]['createTime']);
	    $stmt2->bindParam(3,$arr[$i]['content']);
	    $stmt2->bindParam(4,$arr[$i]['isOriginal']);
	    if($arr[$i]['isOriginal']==0){
		    $stmt2->bindParam(5,$arr[$i]['rAuthor']);
		    $stmt2->bindParam(6,$arr[$i]['rCreateTime']);
		    $stmt2->bindParam(7,$arr[$i]['rContent']);
		}else{
			$flag=0;
			$stmt2->bindParam(5,$flag);
