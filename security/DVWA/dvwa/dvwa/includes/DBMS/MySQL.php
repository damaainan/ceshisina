...
if( !@mysqli_connect( $_DVWA[ 'db_server' ], $_DVWA[ 'db_user' ], $_DVWA[ 'db_password' ] ) ) {
...
if( !@mysqli_query($link, $drop_db ) ) {
	dvwaMessagePush( "Could not drop existing database<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}

$create_db = "CREATE DATABASE {$_DVWA[ 'db_database' ]};";
if( !@mysqli_query($link, $create_db ) ) {
	dvwaMessagePush( "Could not create database<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}
...
// Create table 'users'
if( !@mysqli_select_db($link, $_DVWA[ 'db_database' ] ) ) {
...
if( !mysqli_query($link, $create_tb ) ) {
	dvwaMessagePush( "Table could not be created<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}
...
if( !mysqli_query($link, $insert ) ) {
	dvwaMessagePush( "Data could not be inserted into 'users' table<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}
...
if( !mysqli_query($link, $create_tb_guestbook ) ) {
	dvwaMessagePush( "Table could not be created<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}
...
if( !mysqli_query($link, $insert ) ) {
	dvwaMessagePush( "Data could not be inserted into 'guestbook' table<br />SQL: ".mysqli_error($link) );
	dvwaPageReload();
}
