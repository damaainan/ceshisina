...
[MacPorts][120] 是一个开源的，社区发起的项目，它的目的在于设计一个易于使用的系统，方便编译，安装以及升级 OS X 系统上的 command-line, X11 或者基于 Aqua 的开源软件。
...
[PHP标准组][137] 提出并发布了一系列的风格建议。其中有部分是关于代码风格的，即 [PSR-0][138], [PSR-1][139], [PSR-2][140] 和 [PSR-4][141]。这些推荐只是一些被其他项目所遵循的规则，如 Drupal, Zend, Symfony, CakePHP, phpBB, AWS SDK, FuelPHP, Lithium 等。你可以把这些规则用在自己的项目中，或者继续使用自己的风格。
...
[PEAR][183] 是另一个常用的依赖包管理器, 它跟 Composer 很类似，但是也有一些显著的区别。
...
* [gettext()][237] simply translates a msgid into its corresponding msgstr for a given language. There’s also the shorthand function _() that works the same way;
...
绝大多数时候你的 PHP 程序都需要使用数据库来长久地保存数据。这时你有一些不同的选择可以来连接并与数据库进行交互。在 **PHP 5.1.0 之前**，我们推荐的方式是使用例如 [mysqli][251]，[pgsql][252]，[mssql][253] 等原生驱动。
...
有一些抽象层使用的是[PSR-0][138] 或 [PSR-4][159] 命名空间标准，所以他们可以安装在任何你需要的应用程序中。
...
最后的一个例子是接受外部输入来从文件系统中加载文件。这可以通过将文件名修改为文件路径来进行利用。你需要过滤掉"/", "../", [null 字符][317]或者其他文件路径的字符来确保不会去加载隐藏、私有或者敏感的文件。

* [学习更多数据过滤][318]
* [学习更多 filter_var][319]
* [学习更多 filter_input][320]
* [学习更多 null 字符问题][317]
...
例如，你需要在将外部输入包含在 HTML 中或者插入到原始的 SQL 请求之前对它进行过滤。当你使用 [PDO][321] 中的限制参数功能时，它会自动为你完成过滤的工作。

有些时候你可能需要允许一些安全的 HTML 标签输入进来并被包含在输出的 HTML 页面中，但这实现起来并不容易。尽管有一些像 [HTML Purifier][322] 的白名单类库为了这个原因而出现，实际上更多的人通过使用其他更加严格的格式限制方式例如使用 Markdown 或 BBCode 来避免出现问题。

[查看 Sanitization Filters][323]
...
如果你必须这样做，请你使用 PHP 7 的 [allowed_classes][324] 选项来限制反序列化的对象类型。

### 有效性验证

验证是来确保外部输入的是你所想要的内容。比如，你也许需要在处理注册申请时验证 email 地址、手机号码或者年龄等信息的有效性。

[查看 Validation Filters][325]
...
* [在 PHP 手册中了解 Register_globals][326]
...
> 将值设为 -1 将会显示出所有的错误，甚至包括在未来的 PHP 版本中新增加的类型和参数。 和 PHP 5.4 起开始使用的 E_ALL 是相同的。- [php.net][302]
...
[PHPUnit][332] 是业界PHP应用开发单元测试框架的标准，但也有其他可选的框架：
...
* [Selenium][337]
* [Mink][338]
* [Codeception][339] 是一个全栈的测试框架包括验收性测试工具。
* [Storyplayer][340] 是一个全栈的测试框架并且支持随时创建和销毁测试环境。

## 行为驱动开发

有两种不同的行为驱动开发 (BDD): SpecBDD 和 StoryBDD。 SpecBDD 专注于代码的技术行为，而 StoryBDD 专注于业务逻辑或功能的行为和互动。这两种 BDD 都有对应的 PHP 框架。

采用 StoryBDD 时, 你编写可读的故事来描述应用程序的行为。接着这些故事可以作为应用程序的实际测试案例执行。[Behat][341] 是使用在 PHP 应用程序中的 StoryBDD框架，它受到 Ruby 的 [Cucumber][342] 项目的启发并且实现了 Gherkin DSL 來描述功能的行为。

采用 SpecBDD 时, 你编写规格来描述实际的代码应该有什么行为。你应该描述函数或者方法应该有什么行为，而不是测试函数或者方法。PHP 提供了 [PHPSpec][343] 框架来达到这个目的，这个框架受到了 Ruby 的 [RSpec project][344] 项目的启发。

### BDD 链接

* [Behat][341], PHP 的 StoryBDD 框架， 受到了 Ruby’s [Cucumber][342] 项目的启发。
* [PHPSpec][343], PHP 的 SpecBDD 框架， 受到了 Ruby’s [RSpec][344] 项目的启发。
* [Codeception][339] 是一个使用 BDD 准则的全栈测试框架。
...
* [Selenium][345] 是一个浏览器自动化工具 [integrated with PHPUnit][346]
* [Mockery][347] 是一个可以跟 [PHPUnit][348] 或者 [PHPSpec][343] 整合的 Mock 对象框架
* [Prophecy][349] 是个有自己的想法，且非常强大灵活的 PHP 对象 mocking 框架。它整合了 [PHPSpec][343] 并且可以和 [PHPUnit][348] 一起使用
...
现在，PaaS 已经成为一种部署、托管和扩展各种规模的 PHP 应用程序的流行方式。你可以在 [资源部分][350] 查看 [PHP PaaS “Platform as a Service” 提供商][351] 列表。
...
PHP 通过内置的 FastCGI 进程管理器（FPM），可以很好的与轻量级的高性能 web 服务器 [nginx][352] 协作使用。nginx 比 Apache 占用更少内存而且可以更好的处理并发请求，这对于并没有太多内存的虚拟服务器尤其重要。

* [阅读更多 nginx 的内容][352]
* [阅读更多 PHP-FPM 的内容][353]
* [学习如何配置安全的 nginx 和 PHP-FPM][354]

### Apache 和 PHP

PHP 和 Apache 有很长的合作历史。Apache 有很强的可配置性和大量的 [扩展模块][355] 。是共享主机中常见的Web服务器，完美支持各种 PHP 框架和开源应用(如 WordPress )。可惜的是，默认情况下，Apache 会比 nginx 消耗更多的资源，而且并发处理能力不强。

Apache 有多种方式运行 PHP，最常见的方式就是使用 mode_php5 的 [prefork MPM][356] 方式。但是它对内存的利用效率并不高，如果你不想深入服务器管理方面学习，那么这种简单的方式可能是你最好的选择。需要注意的事如果你使用 mod_php5，就必须使用 prefork MPM。

如果你追求高性能和高稳定性，可以为 Apache 选择与 nginx 类似的的 FPM 系统 [worker MPM][357] 或者 [event MPM][358]，它们分别使用 mod_fastcgi 和 mod_fcgid。这种方式可以更高效的利用内存，运行速度也更快，但是配置也相对复杂一些。

* [阅读更多 Apache][359]
* [阅读更多 Apache 多进程模块][360]
* [阅读更多 mod_fastcgi][361]
* [阅读更多 mod_fcgid][362]
* [阅读更多 mod_proxy_fcgi][363]
* [配置 Apache 通过 mod_proxy_fcgi 使用 PHP-FPM][364]
...
为确保你的共享主机使用了最新的 PHP 版本，请查看：[共享主机 PHP 版本使用][365].

## 构建及部署应用

如果你在手动的进行数据库结构的修改或者在更新文件前手动运行测试，请三思而后行！因为随着每一个额外的手动任务的添加都需要去部署一个新的版本到应用程序，这些更改会增加程序潜在的致命错误。即使你是在处理一个简单的更新，全面的构建处理或者持续集成策略，[构建自动化][366]绝对是你的朋友。
...
[Phing][367] 是一种在 PHP 领域中最简单的开始自动化部署的方式。通过 Phing 你可以控制打包，部署或者测试，只需要一个简单的 XML 构建文件。Phing (基于[Apache Ant][368]) 提供了在安装或者升级 web 应用时的一套丰富的任务脚本，并且可以通过 PHP 编写额外的任务脚本来扩展。

[Capistrano][369] 是一个为 _中高级程序员_ 准备的系统，以一种结构化、可复用的方式在一台或多台远程机器上执行命令。对于部署 Ruby on Rails 的应用，它提供了预定义的配置，不过也可以用它来 **部署 PHP 应用** 。如果要成功的使用 Capistrano ，需要一定的 Ruby 和 Rake 的知识。

对 Capistrano 感兴趣的 PHP 开发者可以阅读 Dave Gardner 的博文 [PHP Deployment with Capistrano][370] ，来作为一个很好的开始。

[Rocketeer][371] 从 Laravel 框架中得到了很多灵感。 目标是默认智能化配置、高速、优雅的自动化部署工具。他支持多服务器，多阶段，并行部署等功能。工具的扩展性极强，并且是由 PHP 编写。

[Deployer][372] 是一个用 PHP 编写的部署工具，它很简单且实用。并行执行任务，原子化部署，在多台服务器之间保持一致性。为 Symfony、Laravel、Zend Framework 和 Yii 提供了通用的任务脚本。推荐阅读 Younes Rafie 的博文 [快速使用 Deployer 部署 PHP 应用][373]。

[Magallanes][374] 是另一个由 PHP 编写的自动化部署工具。使用 YAML 作为配置信息，支持多服务器和多环境，自动化部署。并且自带了许多通用的任务。

#### 延伸阅读：

* [Automate your project with Apache Ant][375]
* [Expert PHP Deployments][376] - free book on deployment with Capistrano, Phing and Vagrant.
* [Deploying PHP Applications][377] - paid book on best practices and tools for PHP deployment.
...
[Ansible][378] 让你使用 YAML 配置文件来管理你的服务器基础设施。简单上手，功能强大，能支持复杂和大型应用场景。甚至支持 API 来动态管理云主机实例。

[Puppet][379] 拥有自定义语言和文件类型来管理服务和配置信息。支持 主从结构或者是 无主结构。在主从结构中，从属机器会在设定周期内更新主机上的配置信息。在无主架构中，你需要 push 推送修改到各个节点。

[Chef][380] 不仅仅只是一个部署框架， 它是一个基于 Ruby 的强大的系统集成框架，除了部署你的应用之外，还可以构建整个服务环境或者虚拟机。AWS 提供一个服务叫 OpsWorks，其集成了 Chef。

#### 延伸阅读：

* [An Ansible Tutorial][381]
* [Ansible for DevOps][382] - paid book on everything Ansible
* [Ansible for AWS][383] - paid book on integrating Ansible and Amazon Web Services
* [Three part blog series about deploying a LAMP application with Chef, Vagrant, and EC2][384]
* [Chef Cookbook which installs and configures PHP and the PEAR package management system][385]
* [Chef video tutorial series][386]
...
对于 PHP 来说，有许多的方式来实现持续集成。近来 [Travis CI][387] 在持续集成上做的很棒，对于小项目来说也可以很好的使用。Travis CI 是一个托管的持续集成服务用于开源社区。它可以和 Github 很好的集成，并且提供了很多语言的支持包括 PHP 。

#### 延伸阅读：

* [使用 Jenkins 进行持续集成][388]
* [使用 PHPCI 进行持续集成][389]
* [使用 Teamcity 进行持续集成][390]
...
[Vagrant][391] 可以让你使用单一的配置信息来部署一套虚拟环境, 最后打包为一个所谓的 box (就是已经部署好环境的虚拟机器). 你可以手动来安装和配置 box, 也可以使用自动部署工具, 如 [Puppet][392] 或者 [Chef][380] .
...
* [Rove][393]: 使用 Chef 自动化安装一些常用的软件, PHP 包含在内.
* [Puphpet][394]: 简单的 Web 图形界面用来生成部署 PHP 环境的 Puppet 脚本, 此项目不仅可以用在开发上, 也可以在生产环境中使用.
* [Protobox][395]: 是一个基于 vagrant 的一个层, 还有 Web 图形界面, 允许你使用一个 YAML 文件来安装和配置虚拟机里面的软件.
* [Phansible][396]: 提供了一个简单的 Web 图形界面, 用来创建 Ansible 自动化部署脚本, 专门为 PHP 项目定制.

## Docker 简介

[Docker][397] - 一个轻量级的虚拟化解决方案。Docker 为各种应用程序提供了 Linux 容器。 A container is a building block which, in the simplest case, does one specific job, e.g. running a web server. An “image” is the package you use to build the container - Docker has a repository full of them.
...
在你成功 [安装 Docker][398] 后, 你只需要一步就可以安装 Apache + PHP.
...
以上的命令教你快速的创建简单的服务器。Docker 还提供了很多功能等着你去发现。[Docker Hub][399]) 上更是提供了数以千计的已经构建好的镜像。花点时间去学习下 [Docker 用户手册][400]。不要运行随机看到的代码，非官方的镜像有时候并没有最新安全更新，如果你对镜像有疑问，优先选择 [官方的仓库][401]。

[PHPDocker.io][402] 是另一个很棒的 PHP 相关的 Docker 资源站点。支持自动生成一个全栈的 LAMP/LEMP 服务器，包含你自定义的 PHP 版本和扩展。

* [Docker Website][397]
* [Docker Installation][398]
* [Docker User Guide][400]
* [Docker Hub][399]
* [Docker Hub - official images][401]
...
当一个 PHP 文件被解释执行的时候，首先是被编译成名为 [opcodes][403] 的中间代码，然后才被底层的虚拟机执行。 如果PHP文件没有被修改过，opcode 始终是一样的。这就意味着编译步骤白白浪费了 CPU 的资源。

此时 opcode 缓存就派上用场了。通过将 opcode 缓存在内存中，它能防止冗余的编译步骤，并且在下次调用执行时得到重用。设置 opcode 缓存只需要几分钟的时间，你的应用程序便会因此大大加速，实在没有理由不用它。

PHP 5.5 中自带了 opcode 缓存工具，叫做[Zend OPcache][404]，默认一般是开启的，请在 phpinfo() 输出中检查 [opcache.enable][405] 关键词是否出现来确定是否开启。早期的版本也能通过 PECL 扩展来安装。

更多关于 opcode 缓存的资料：

* [Zend OPcache][404] (bundled with PHP since 5.5)
* Zend OPcache (formerly known as Zend Optimizer+) is now [open source][406]
* [APC][407] - PHP 5.4 and earlier
* [XCache][408]
* [WinCache][409] (extension for MS Windows Server)
* [list of PHP accelerators on Wikipedia][410]
...
PHPDoc 是注释 PHP 代码的非正式标准。它有_许多_不同的[标记][417]可以使用。完整的标记列表和范例可以查看 [PHPDoc 指南][418]。
...
这个类的说明使用了 [@author][419] 和 [@link][420]标记， [@author][419] 标记是用來说明代码的作者，在多位开发者的情况下，可以同时列出好几位。其次 [@link][420] 标记用来提供网站链接，进一步说明代码和网站之间的关系。

在这个类中，第一个方法的 [@param][421] 标记，说明类型、名字和传入方法的参数。此外，[@return][422] 和 [@throws][423] 标记说明返回类型以及可能抛出的异常。

第二、第三个方法非常类似，和第一个方法一样使用一个 [@param][421] 标记。第二、和第三个方法之间关键差別在注释区块使用／排除 [@return][422] 标记。@return void 标记明确告诉我们没有返回值，而过去省略 @return void 声明也具有相同效果（沒有返回任何值）。
...
* [New Relic: 25 PHP Developers to Follow Online][426]
* [OGProgrammer: How to get connected with the PHP community][427]

## 指导

* [php-mentoring.org][428] - PHP 社区中的一对一指导。
...
* [Heroku][431]
* [fortrabbit][432]
* [Engine Yard Cloud][433]
* [Red Hat OpenShift Platform][434]
* [AWS Elastic Beanstalk][435]
* [Windows Azure][436]
* [Google App Engine][437]
* [Jelastic][438]
* [Platform.sh][439]
* [Cloudways][440]
* [IBM Bluemix Cloud Foundry][441]
* [Pivotal Web Service Cloud Foundry][442]

To see which versions these PaaS hosts are running, head over to [PHP Versions][443].
...
* [Hoa Project][447]
* [Orno][448]
* [Symfony Components][449]
* [The League of Extraordinary Packages][450]
...
* [Laravel 中文书籍《Laravel 入门教程》][454]
* [下载量最高的 100 个 Laravel 扩展包推荐][455]
* [Laravel 中文文档，涵盖 5.1, 5.2, 5.3 版本][456]
* [Laravel 招聘][457]
* [Laravel 远程工作][458]
* [Laravel 中文社区][1] 我们是现代化 PHP 的拥护者
* [Laravel 速查表][459]
...
* [PHP Cheatsheets][460] - for variable comparisons, arithmetics and variable testing in various PHP versions
* [PHP Security Cheatsheet][461]

### 更多最佳实践

* [PHP Best Practices][462]
* [Best practices for Modern PHP Development][463]
...
* [PHP Weekly][464]
* [JavaScript Weekly][465]
* [HTML5 Weekly][466]
* [Mobile Web Weekly][467]
* There are also Weeklies on other platforms you might be interested in; here’s [a list of some][468].

### PHP 世界

* [PHP Developer blog][469]
...
* [Standards and Best practices][474]
* [PHP Training on Pluralsight][475]
* [PHP Training on Lynda.com][476]
* [PHP Training on Tutsplus][477]
* [Laracasts][478]
...
* [PHP Pandas][479] - Web 开发入门书籍
* [PHP The Right Way][480] - 本书完全免费
* [Using Libsodium in PHP Projects][481] - 使用 Libsodium 来构建现代的、安全的、快速的 PHP 加密

### 付费书籍

* [Laravel 中文书籍《Laravel 入门教程》][454] - Web 开发实战书籍，一步步构建一个微博应用
* [Build APIs You Won’t Hate][482] - 教你构建 API
* [Modern PHP][483] - 现代化 PHP
* [Building Secure PHP Apps][484] - 构建安全的 PHP 应用
* [Modernizing Legacy Applications In PHP][485] - 小步迭代的升级你的 PHP 项目
* [Securing PHP: Core Concepts][486] - PHP 安全核心概念
* [Scaling PHP][487] - 高可用性 PHP
* [Signaling PHP][488] - PHP 命令行实战
* [The Grumpy Programmer’s Guide To Building Testable PHP Applications][489] - PHP 测试指南
* [Minimum Viable Tests][490] - 小步迭代开始学习 PHP 测试
* [Domain-Driven Design in PHP][491] - PHP 的 DDD 设计模式详解

[回到顶部][110]

# 社区

PHP 社区多元化并且规模庞大，成员们也乐意并随时准备好帮助新人。你可以考虑加入当地的 PHP 使用者社区 （PUG） 或者参加教大型的 PHP 会议，从中学习更多最佳实践。你也可以使用 IRC 逛逛 [irc.freenode.com][492] 上的 #phpc 频道，也可以关注 [@phpc][493] 的Twitter 账号。试着去多结交一些新的开发者，学习新的东西，总之，交一些新朋友！其他的社区资源包含 Google+ 的 PHP [Programmer community][494] 以及 [StackOverflow][495]。

中文的 PHP 开发者，欢迎加入 [Laravel China 社区][1] （简称 LC）。LC 是 PHP 和 Laravel 开发者社区，致力于为 PHP 和 Laravel 开发者提供一个分享创造、结识伙伴、协同互助的平台。LC 是现代化 PHP 的拥护者。LC 也是这个 PHP The Right Way 中文文档的维护者。

[阅读 PHP 官方事件日历][496]

## PHP 用户群

如果你住在较大的城市，附近应该就有 PHP 用户群。你可以通过基于 [PHP.ug][497] 的 [usergroup-list at php.net][498] 这个地址找到当地的 PUG。也可以通过 [Meetup.com][499] 或者使用搜索引擎 (i.e. [Google][500]) 搜索 php user group near me 。如果你住在比较小的城镇，当地也许还没有 PUG ，如果是这种情形，不妨就开始组建一个。

这里要特别提到两个全球的用户组：[NomadPHP][501] 和 [PHPWomen][502]。[NomadPHP][501] 提供每月两次的在线用户组会议，由 PHP 社区里顶尖的高手进行演讲。[PHPWomen][502] 原本是针对女性 PHP 开发者的非排他性的用户组。会员资格发放给那些支持多元化社区的人。PHPWomen 提供了一技术支持，指导和教育的个平台，并且促进了女性的创造力以及专业的氛围。

[了解关于 PHP Wiki 上的用户群][503]
...
[ElePHPant][505] is that beautiful mascot of the PHP project with elephant in their design. It was originally designed for the PHP project in 1998 by [Vincent Pontier][506] - spiritual father of thousands of elePHPants around the world and 10 years later adorable plush elephant toy came to birth as well. Now elePHPants are present at many PHP conferences and with many PHP developers at their computers for fun and inspiration.

[Interview with Vincent Pontier][507]

[0]: /
[1]: https://laravel-china.org/
[2]: http://service.weibo.com/share/share.php?url=https://laravel-china.github.io/php-the-right-way&title=PHP+The+Right+Way+PHP+之道
[3]: /php-the-right-way#welcome
[4]: /php-the-right-way#translations
[5]: /php-the-right-way#how-to-contribute
[6]: /php-the-right-way#spread-the-word
[7]: /php-the-right-way/#getting_started
[8]: /php-the-right-way/#use_the_current_stable_version
[9]: /php-the-right-way/#builtin_web_server
[10]: /php-the-right-way/#mac_setup
[11]: /php-the-right-way/#windows_setup
[12]: /php-the-right-way/#code_style_guide
[13]: /php-the-right-way/#language_highlights
[14]: /php-the-right-way/#programming_paradigms
[15]: /php-the-right-way/#namespaces
[16]: /php-the-right-way/#standard_php_library
[17]: /php-the-right-way/#command_line_interface
[18]: /php-the-right-way/#xdebug
[19]: /php-the-right-way/#dependency_management
[20]: /php-the-right-way/#composer_and_packagist
[21]: /php-the-right-way/#pear
[22]: /php-the-right-way/#coding_practices
[23]: /php-the-right-way/#the_basics
[24]: /php-the-right-way/#date_and_time
[25]: /php-the-right-way/#design_patterns
[26]: /php-the-right-way/#php_and_utf8
[27]: /php-the-right-way/#i18n_l10n
[28]: /php-the-right-way/#dependency_injection
[29]: /php-the-right-way/#basic_concept
[30]: /php-the-right-way/#complex_problem
[31]: /php-the-right-way/#containers
[32]: /php-the-right-way/#further_reading
[33]: /php-the-right-way/#databases
[34]: /php-the-right-way/#mysql_extension
[35]: /php-the-right-way/#pdo_extension
[36]: /php-the-right-way/#databases_interacting
[37]: /php-the-right-way/#databases_abstraction_layers
[38]: /php-the-right-way/#templating
[39]: /php-the-right-way/#templating_benefits
[40]: /php-the-right-way/#plain_php_templates
[41]: /php-the-right-way/#compiled_templates
[42]: /php-the-right-way/#templating_further_reading
[43]: /php-the-right-way/#errors_and_exceptions
[44]: /php-the-right-way/#errors
[45]: /php-the-right-way/#exceptions
[46]: /php-the-right-way/#security
[47]: /php-the-right-way/#web_application_security
[48]: /php-the-right-way/#password_hashing
[49]: /php-the-right-way/#data_filtering
[50]: /php-the-right-way/#configuration_files
[51]: /php-the-right-way/#register_globals
[52]: /php-the-right-way/#error_reporting
[53]: /php-the-right-way/#testing
[54]: /php-the-right-way/#test_driven_development
[55]: /php-the-right-way/#behavior_driven_development
[56]: /php-the-right-way/#complementary_testing_tools
[57]: /php-the-right-way/#servers_and_deployment
[58]: /php-the-right-way/#platform_as_a_service
[59]: /php-the-right-way/#virtual_or_dedicated_servers
[60]: /php-the-right-way/#shared_servers
[61]: /php-the-right-way/#building_and_deploying_your_application
[62]: /php-the-right-way/#virtualization
[63]: /php-the-right-way/#vagrant
[64]: /php-the-right-way/#docker
[65]: /php-the-right-way/#caching
[66]: /php-the-right-way/#opcode_cache
[67]: /php-the-right-way/#object_caching
[68]: /php-the-right-way/#documenting
[69]: /php-the-right-way/#phpdoc
[70]: /php-the-right-way/#resources
[71]: /php-the-right-way/#from_the_source
[72]: /php-the-right-way/#people_to_follow
[73]: /php-the-right-way/#mentoring
[74]: /php-the-right-way/#php_paas_providers
[75]: /php-the-right-way/#frameworks
[76]: /php-the-right-way/#components
[77]: /php-the-right-way/#other_resources
[78]: /php-the-right-way/#videos
[79]: /php-the-right-way/#books
[80]: /php-the-right-way/#community
[81]: /php-the-right-way/#user_groups
[82]: /php-the-right-way/#conferences
[83]: /php-the-right-way/#elephpants
[84]: /php-the-right-way/#credits
[85]: http://www.phptherightway.com
[86]: http://rwetzlmayr.github.io/php-the-right-way
[87]: http://phpdevenezuela.github.io/php-the-right-way
[88]: http://eilgin.github.io/php-the-right-way/
[89]: http://id.phptherightway.com
[90]: http://it.phptherightway.com
[91]: http://pl.phptherightway.com
[92]: http://br.phptherightway.com
[93]: https://bgui.github.io/php-the-right-way/
[94]: http://sl.phptherightway.com
[95]: http://phpsrbija.github.io/php-the-right-way/
[96]: http://hkulekci.github.io/php-the-right-way/
[97]: http://bg.phptherightway.com
[98]: http://getjump.github.io/ru-php-the-right-way
[99]: http://iflista.github.com/php-the-right-way
[100]: https://adaroobi.github.io/php-the-right-way/
[101]: http://novid.github.io/php-the-right-way/
[102]: https://apzentral.github.io/php-the-right-way/
[103]: http://modernpug.github.io/php-the-right-way
[104]: http://ja.phptherightway.com
[105]: https://laravel-china.github.io/php-the-right-way/
[106]: http://laravel-taiwan.github.io/php-the-right-way
[107]: https://leanpub.com/phptherightway
[108]: https://github.com/summerblue/php-the-right-way/tree/gh-pages
[109]: /banners.html
[110]: #top
[111]: http://php.net/downloads.php
[112]: #language_highlights
[113]: http://php.net/supported-versions.php
[114]: http://php.net/manual/migration71.incompatible.php
[115]: http://php.net/manual/
[116]: http://php.net/features.commandline.webserver
[117]: http://brew.sh/
[118]: https://github.com/Homebrew/homebrew-php#installation
[119]: https://github.com/philcook/brew-php-switcher
[120]: https://www.macports.org/install.php
[121]: https://github.com/phpbrew/phpbrew
[122]: http://php-osx.liip.ch/
[123]: http://php.net/install.macosx.compile
[124]: https://github.com/kennethreitz/osx-gcc-installer
[125]: https://developer.apple.com/downloads
[126]: http://www.mamp.info/en/downloads/
[127]: http://www.apachefriends.org/en/xampp.html
[128]: http://windows.php.net/download/
[129]: http://www.windows-commandline.com/set-path-command-line/
[130]: https://www.microsoft.com/web/downloads/platform.aspx
[131]: http://www.easyphp.org/
[132]: http://www.wampserver.com/en/
[133]: http://phpmanager.codeplex.com/
[134]: http://php.iis.net/
[135]: /#virtualization_title
[136]: http://ctankersley.com/2016/11/13/developing-on-windows-2016/
[137]: https://psr.phphub.org/
[138]: http://www.php-fig.org/psr/psr-0/
[139]: https://laravel-china.org/topics/2078
[140]: https://laravel-china.org/topics/2079
[141]: https://laravel-china.org/topics/2081
[142]: http://pear.php.net/manual/en/standards.php
[143]: http://symfony.com/doc/current/contributing/code/standards.html
[144]: http://pear.php.net/package/PHP_CodeSniffer/
[145]: https://github.com/benmatselby/sublime-phpcs
[146]: http://cs.sensiolabs.org/
[147]: https://github.com/squizlabs/PHP_CodeSniffer/wiki/Fixing-Errors-Automatically
[148]: http://php.net/language.oop5
[149]: http://php.net/language.oop5.traits
[150]: /php-the-right-way/pages/Functional-Programming.html
[151]: http://php.net/functions.anonymous
[152]: http://php.net/class.closure
[153]: https://wiki.php.net/rfc/closures
[154]: http://php.net/language.types.callable
[155]: http://php.net/function.call-user-func-array
[156]: http://php.net/language.oop5.magic
[157]: http://php.net/intro.reflection
[158]: http://php.net/language.oop5.overloading
[159]: http://www.php-fig.org/psr/psr-4/
[160]: http://php.net/language.namespaces
[161]: http://php.net/book.spl
[162]: http://www.lynda.com/PHP-tutorials/Up-Running-Standard-PHP-Library/175038-2.html
[163]: http://php.net/function.phpinfo
[164]: http://php.net/features.commandline.options
[165]: http://php.net/reserved.variables.argc
[166]: http://php.net/reserved.variables.argv
[167]: http://www.gsp.com/cgi-bin/man.cgi?section=3&topic=sysexits
[168]: http://php.net/features.commandline
[169]: http://php.net/install.windows.commandline
[170]: http://xdebug.org/docs/install
[171]: http://xdebug.org/docs/
[172]: http://www.bluestatic.org/software/macgdbp/
[173]: /#composer_and_packagist
[174]: /#pear
[175]: http://packagist.org/
[176]: https://laravel-china.org/composer
[177]: https://getcomposer.org/download/
[178]: https://getcomposer.org/Composer-Setup.exe
[179]: http://twig.sensiolabs.org
[180]: https://www.versioneye.com/
[181]: https://security.sensiolabs.org/
[182]: http://getcomposer.org/doc/00-intro.md
[183]: http://pear.php.net/
[184]: http://pear.php.net/manual/en/installation.getting.php
[185]: http://pear.php.net/packages.php
[186]: http://pear.php.net/manual/en/guide.users.commandline.channels.php
[187]: http://getcomposer.org/doc/05-repositories.md#pear
[188]: /php-the-right-way/pages/The-Basics.html
[189]: http://carbon.nesbot.com
[190]: http://php.net/book.datetime
[191]: http://php.net/function.date
[192]: /php-the-right-way/pages/Design-Patterns.html
[193]: https://alexcabal.com/
[194]: https://phpbestpractices.org/#utf-8
[195]: http://php.net/book.mbstring
[196]: https://packagist.org/packages/patchwork/utf8
[197]: http://htmlpurifier.org/docs/enduser-utf8.html
[198]: https://developers.google.com/speed/docs/best-practices/rendering#SpecifyCharsetEarly
[199]: http://php.net/language.operators.string
[200]: http://php.net/ref.strings
[201]: http://php.net/function.strpos
[202]: http://php.net/function.strlen
[203]: http://php.net/function.substr
[204]: http://php.net/ref.mbstring
[205]: http://php.net/function.mb-strpos
[206]: http://php.net/function.mb-strlen
[207]: http://php.net/function.mb-substr
[208]: http://php.net/function.mb-internal-encoding
[209]: http://php.net/function.mb-http-output
[210]: http://php.net/function.htmlentities
[211]: http://php.net/function.htmlspecialchars
[212]: http://blog.loftdigital.com/blog/php-utf-8-cheatsheet
[213]: http://www.phpwact.org/php/i18n/utf-8
[214]: http://stackoverflow.com/questions/571694/what-factors-make-php-unicode-incompatible
[215]: http://stackoverflow.com/questions/140728/best-practices-in-php-and-mysql-with-international-strings
[216]: http://mathiasbynens.be/notes/mysql-utf8mb4
[217]: http://www.sitepoint.com/bringing-unicode-to-php-with-portable-utf8/
[218]: http://stackoverflow.com/questions/8218230/php-domdocument-loadhtml-not-encoding-utf-8-correctly
[219]: https://en.wikipedia.org/wiki/Gettext
[220]: https://github.com/oscarotero/Gettext
[221]: https://symfony.com/doc/current/components/translation.html
[222]: https://docs.zendframework.com/zend-i18n/translation
[223]: https://laravel.com/docs/master/localization
[224]: http://www.yiiframework.com/doc-2.0/guide-tutorial-i18n.html
[225]: http://br2.php.net/manual/en/intro.intl.php
[226]: http://www.icu-project.org
[227]: https://poedit.net
[228]: https://poedit.net/download
[229]: https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes
[230]: http://en.wikipedia.org/wiki/ISO_3166-1_alpha-2
[231]: http://www.gnu.org/software/gettext/manual/gettext.html#Rare-Language-Codes
[232]: http://docs.translatehouse.org/projects/localization-guide/en/latest/l10n/pluralforms.html
[233]: https://lingohub.com/blog/2013/07/php-internationalization-with-gettext-tutorial/#Plurals
[234]: http://php.net/manual/en/function.sprintf.php
[235]: http://www.gnu.org/software/gettext/manual/gettext.html
[236]: https://symfony.com/doc/current/components/translation/usage.html#creating-translations
[237]: http://php.net/manual/en/function.gettext.php
[238]: http://php.net/manual/en/function.ngettext.php
[239]: http://php.net/manual/en/function.dgettext.php
[240]: http://php.net/manual/en/function.dngettext.php
[241]: https://www.gnu.org/software/gettext/manual/gettext.html#Language-specific-options
[242]: https://en.wikipedia.org/wiki/Internationalization_and_localization
[243]: https://lingohub.com/blog/2013/07/php-internationalization-with-gettext-tutorial/
[244]: http://php.net/manual/en/book.gettext.php
[245]: http://en.wikipedia.org/wiki/Dependency_injection
[246]: http://ralphschindler.com/2011/05/18/learning-about-dependency-injection-and-php
[247]: http://fabien.potencier.org/article/11/what-is-dependency-injection
[248]: https://mwop.net/blog/260-Dependency-Injection-An-analogy.html
[249]: http://net.tutsplus.com/tutorials/php/dependency-injection-huh/
[250]: http://philipobenito.github.io/dependency-injection-as-a-tool-for-testing/
[251]: http://php.net/mysqli
[252]: http://php.net/pgsql
[253]: http://php.net/mssql
[254]: http://php.net/mysql
[255]: http://php.net/pdo
[256]: http://php.net/migration55.deprecated
[257]: http://php.net/manual/en/migration70.removed-exts-sapis.php
[258]: http://php.net/mysqlinfo.api.choosing
[259]: http://wiki.hashphp.org/PDO_Tutorial_for_MySQL_Developers
[260]: #data_filtering
[261]: http://php.net/book.pdo
[262]: http://php.net/pdo.connections
[263]: /#object-oriented-programming
[264]: /#functional-programming
[265]: http://code.tutsplus.com/tutorials/mvc-for-noobs--net-10488
[266]: /#frameworks
[267]: /#unit-testing
[268]: http://phpbridge.org/
[269]: http://phpbridge.org/intro-to-php/creating_a_data_class
[270]: https://github.com/auraphp/Aura.Sql
[271]: http://www.doctrine-project.org/projects/dbal.html
[272]: http://propelorm.org/
[273]: https://packages.zendframework.com/docs/latest/manual/en/index.html#zendframework/zend-db
[274]: /pages/Design-Patterns.html#model-view-controller
[275]: http://platesphp.com/
[276]: https://github.com/auraphp/Aura.View
[277]: http://fabien.potencier.org/article/34/templating-engines-in-php
[278]: https://github.com/box/brainy
[279]: http://www.smarty.net/
[280]: http://mustache.github.io/
[281]: http://twig.sensiolabs.org/
[282]: http://code.tutsplus.com/tutorials/an-introduction-to-views-templating-in-codeigniter--net-25648
[283]: http://www.smashingmagazine.com/2011/10/17/getting-started-with-php-templating/
[284]: http://code.tutsplus.com/tutorials/roll-your-own-templating-system-in-php--net-16596
[285]: https://laracasts.com/series/laravel-from-scratch/episodes/7
[286]: http://code.tutsplus.com/tutorials/working-with-templates-in-symfony-2--cms-21172
[287]: https://github.com/box/brainy/wiki/Writing-Safe-Templates
[288]: http://laravel.com/docs/blade
[289]: http://dwoo.org/
[290]: https://github.com/nette/latte
[291]: https://github.com/bobthecow/mustache.php
[292]: http://phptal.org/
[293]: http://framework.zend.com/manual/2.3/en/modules/zend.view.quick-start.html
[294]: errorreport
[295]: http://xdebug.org/docs/basic
[296]: http://php.net/language.operators.errorcontrol
[297]: http://www.sitepoint.com/
[298]: http://php.net/book.scream
[299]: http://filp.github.io/whoops/
[300]: errorexception
[301]: http://php.net/errorfunc.constants
[302]: http://php.net/function.error-reporting
[303]: /#error_reporting
[304]: /#standard_php_library
[305]: http://php.net/language.exceptions
[306]: http://php.net/spl.exceptions
[307]: http://www.brandonsavage.net/exceptional-php-nesting-exceptions-in-php/
[308]: http://ralphschindler.com/2010/09/15/exception-best-practices-in-php-5-3
[309]: https://www.owasp.org/
[310]: http://phpsecurity.readthedocs.org/en/latest/index.html
[311]: https://www.owasp.org/index.php/Guide_Table_of_Contents
[312]: http://en.wikipedia.org/wiki/Cryptographic_hash_function
[313]: https://en.wikipedia.org/wiki/Salt_(cryptography)
[314]: http://php.net/function.password-hash
[315]: https://github.com/ircmaxell/password_compat
[316]: https://wiki.php.net/rfc/password_hash
[317]: http://php.net/security.filesystem.nullbytes
[318]: http://php.net/book.filter
[319]: http://php.net/function.filter-var
[320]: http://php.net/function.filter-input
[321]: #databases
[322]: http://htmlpurifier.org/
[323]: http://php.net/filter.filters.sanitize
[324]: https://secure.php.net/manual/en/function.unserialize.php
[325]: http://php.net/filter.filters.validate
[326]: http://php.net/security.globals
[327]: http://php.net/errorfunc.configuration#ini.error-reporting
[328]: http://php.net/errorfunc.configuration#ini.display-errors
[329]: http://php.net/errorfunc.configuration#ini.display-startup-errors
[330]: http://php.net/errorfunc.configuration#ini.log-errors
[331]: http://en.wikipedia.org/wiki/Test-driven_development
[332]: https://phpunit.de/
[333]: https://github.com/atoum/atoum
[334]: https://github.com/crysalead/kahlan
[335]: http://peridot-php.github.io/
[336]: http://simpletest.org
[337]: http://docs.seleniumhq.org/
[338]: http://mink.behat.org/en/latest/
[339]: http://codeception.com/
[340]: http://datasift.github.io/storyplayer/
[341]: http://behat.org/
[342]: http://cukes.info/
[343]: http://www.phpspec.net/
[344]: http://rspec.info/
[345]: http://seleniumhq.org/
[346]: https://github.com/giorgiosironi/phpunit-selenium/
[347]: https://github.com/padraic/mockery
[348]: http://phpunit.de/
[349]: https://github.com/phpspec/prophecy
[350]: #resources
[351]: #php_paas_providers
[352]: http://nginx.org/
[353]: http://php.net/install.fpm
[354]: https://nealpoole.com/blog/2011/04/setting-up-php-fastcgi-and-nginx-dont-trust-the-tutorials-check-your-configuration/
[355]: http://httpd.apache.org/docs/2.4/mod/
[356]: http://httpd.apache.org/docs/2.4/mod/prefork.html
[357]: http://httpd.apache.org/docs/2.4/mod/worker.html
[358]: http://httpd.apache.org/docs/2.4/mod/event.html
[359]: http://httpd.apache.org/
[360]: http://httpd.apache.org/docs/2.4/mod/mpm_common.html
[361]: https://blogs.oracle.com/opal/entry/php_fpm_fastcgi_process_manager
[362]: http://httpd.apache.org/mod_fcgid/
[363]: https://httpd.apache.org/docs/current/mod/mod_proxy_fcgi.html
[364]: https://serversforhackers.com/video/apache-and-php-fpm
[365]: http://phpversions.info/shared-hosting/
[366]: http://en.wikipedia.org/wiki/Build_automation
[367]: http://www.phing.info/
[368]: http://ant.apache.org/
[369]: https://github.com/capistrano/capistrano/wiki
[370]: http://www.davegardner.me.uk/blog/2012/02/13/php-deployment-with-capistrano/
[371]: http://rocketeer.autopergamene.eu/
[372]: http://deployer.org/
[373]: http://www.sitepoint.com/deploying-php-applications-with-deployer/
[374]: http://magephp.com/
[375]: http://net.tutsplus.com/tutorials/other/automate-your-projects-with-apache-ant/
[376]: http://viccherubini.com/assets/Expert-PHP-Deployments.pdf
[377]: http://www.deployingphpapplications.com
[378]: https://www.ansible.com/
[379]: https://puppet.com/
[380]: https://www.chef.io/
[381]: https://serversforhackers.com/an-ansible-tutorial
[382]: https://leanpub.com/ansible-for-devops
[383]: https://leanpub.com/ansible-for-aws
[384]: http://www.jasongrimes.org/2012/06/managing-lamp-environments-with-chef-vagrant-and-ec2-1-of-3/
[385]: https://github.com/chef-cookbooks/php
[386]: https://www.youtube.com/playlist?list=PL11cZfNdwNyPnZA9D1MbVqldGuOWqbumZ
[387]: https://travis-ci.org/
[388]: http://jenkins-ci.org/
[389]: http://www.phptesting.org/
[390]: http://www.jetbrains.com/teamcity/
[391]: http://vagrantup.com/
[392]: http://www.puppetlabs.com/
[393]: http://rove.io/
[394]: https://puphpet.com/
[395]: http://getprotobox.com/
[396]: http://phansible.com/
[397]: http://docker.com/
[398]: https://docs.docker.com/installation/
[399]: https://hub.docker.com/
[400]: https://docs.docker.com/userguide/
[401]: https://hub.docker.com/explore/
[402]: https://phpdocker.io/generator
[403]: http://php.net/manual/en/internals2.opcodes.php
[404]: http://php.net/book.opcache
[405]: http://php.net/manual/en/opcache.configuration.php#ini.opcache.enable
[406]: https://github.com/zendtech/ZendOptimizerPlus
[407]: http://php.net/book.apc
[408]: http://xcache.lighttpd.net/
[409]: http://www.iis.net/download/wincacheforphp
[410]: http://en.wikipedia.org/wiki/List_of_PHP_accelerators
[411]: https://github.com/krakjoe/apcu
[412]: http://php.net/ref.apc
[413]: http://memcached.org/
[414]: http://redis.io/
[415]: http://xcache.lighttpd.net/wiki/XcacheApi
[416]: http://php.net/ref.wincache
[417]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/index.html
[418]: http://www.phpdoc.org/docs/latest/index.html
[419]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/author.html
[420]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/link.html
[421]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/param.html
[422]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/return.html
[423]: http://www.phpdoc.org/docs/latest/references/phpdoc/tags/throws.html
[424]: http://php.net/
[425]: http://php.net/docs.php
[426]: https://blog.newrelic.com/2014/05/02/25-php-developers-follow-online/
[427]: https://www.ogprogrammer.com/2017/06/28/how-to-get-connected-with-the-php-community/
[428]: http://php-mentoring.org/
[429]: https://pagodabox.io/
[430]: https://www.ctl.io/appfog/
[431]: https://devcenter.heroku.com/categories/php
[432]: https://www.fortrabbit.com/
[433]: https://www.engineyard.com/features
[434]: https://www.openshift.com/
[435]: https://aws.amazon.com/elasticbeanstalk/
[436]: http://www.windowsazure.com/
[437]: https://cloud.google.com/appengine/docs/php/
[438]: http://jelastic.com/
[439]: https://platform.sh/
[440]: https://www.cloudways.com/en/
[441]: https://console.ng.bluemix.net/
[442]: https://run.pivotal.io/
[443]: http://phpversions.info/paas-hosting/
[444]: https://github.com/codeguy/php-the-right-way/wiki/Frameworks
[445]: http://auraphp.com/framework/2.x/en/
[446]: https://github.com/fuelphp
[447]: https://github.com/hoaproject
[448]: https://github.com/orno
[449]: http://symfony.com/doc/current/components/index.html
[450]: http://thephpleague.com/
[451]: https://github.com/illuminate/container
[452]: https://github.com/illuminate/database
[453]: https://github.com/illuminate/queue
[454]: https://laravel-china.org/topics/3383
[455]: https://laravel-china.org/topics/2530
[456]: https://laravel-china.org/docs/home
[457]: https://laravel-china.org/categories/1
[458]: https://laravel-china.org/topics/3626
[459]: https://cs.laravel-china.org/
[460]: http://phpcheatsheets.com/
[461]: https://www.owasp.org/index.php/PHP_Security_Cheat_Sheet
[462]: https://phpbestpractices.org/
[463]: https://www.airpair.com/php/posts/best-practices-for-modern-php-development
[464]: http://www.phpweekly.com
[465]: http://javascriptweekly.com
[466]: http://html5weekly.com
[467]: http://mobilewebweekly.co
[468]: https://github.com/jondot/awesome-weekly
[469]: http://blog.phpdeveloper.org/
[470]: https://www.youtube.com/user/phpacademy
[471]: https://www.youtube.com/user/thenewboston
[472]: https://www.youtube.com/user/businessgeek
[473]: https://www.youtube.com/user/LevelUpTuts
[474]: http://teamtreehouse.com/library/standards-and-best-practices
[475]: http://www.pluralsight.com/search/?searchTerm=php
[476]: http://www.lynda.com/search?q=php
[477]: http://code.tutsplus.com/categories/php/courses
[478]: https://laracasts.com/
[479]: http://daylerees.com/php-pandas/
[480]: https://leanpub.com/phptherightway/
[481]: https://paragonie.com/book/pecl-libsodium
[482]: https://apisyouwonthate.com/
[483]: http://shop.oreilly.com/product/0636920033868.do
[484]: https://leanpub.com/buildingsecurephpapps
[485]: https://leanpub.com/mlaphp
[486]: https://leanpub.com/securingphp-coreconcepts
[487]: http://www.scalingphpbook.com/
[488]: https://leanpub.com/signalingphp
[489]: https://leanpub.com/grumpy-testing
[490]: https://leanpub.com/minimumviabletests
[491]: https://leanpub.com/ddd-in-php
[492]: http://webchat.freenode.net/?channels=phpc
[493]: https://twitter.com/phpc
[494]: https://plus.google.com/u/0/communities/104245651975268426012
[495]: http://stackoverflow.com/questions/tagged/php
[496]: http://php.net/cal.php
[497]: http://php.ug/
[498]: http://php.net/ug.php
[499]: http://www.meetup.com/find/
[500]: https://www.google.com/search?q=php+user+group+near+me
[501]: https://nomadphp.com/
[502]: http://phpwomen.org/
[503]: https://wiki.php.net/usergroups
[504]: http://php.net/conferences/index.php
[505]: http://php.net/elephpant.php
[506]: http://www.elroubio.net/
[507]: http://7php.com/elephpant/