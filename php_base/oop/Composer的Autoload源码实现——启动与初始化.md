...
这个数组，然后我们就会遍历这个数组来和`Parsedown/example`比较，发现第一个Prophecy不符合，第二个Parsedown符合，然后得到了映射目录：( **映射目录可能不止一个** ) 
...
具体的用法：假如我们找`Symfony\Polyfill\Mbstring\example`这个命名空间，和PSR0一样通过前缀索引和字符串匹配我们得到了
