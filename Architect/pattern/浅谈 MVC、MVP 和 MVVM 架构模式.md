...
12 Jul 2017 on [MVC][0], [iOS][1]

* [谈谈 MVX 中的 Model][2]
* [谈谈 MVX 中的 View][3]
* [谈谈 MVX 中的 Controller][4]
* [浅谈 MVC、MVP 和 MVVM 架构模式][5]
...
> 更早的概念类似 Model-View-Editor（MVE）这里就不再提及了，感兴趣的读者可以阅读这篇论文 [Thing-Model-View-Editor][8] 了解更多的信息。
...
在最近的几个月，作者发现不同人对于 MVC 的理解有巨大的差异，这是 [ASP.NET MVC Overview][9] 一文中对于 MVC 模式描述的示意图。
...
1. 视图层和模型层之间没有直接关系，只有间接关系，通过控制器对模型进行查询、返回给 DispatchServlet 后再传递至视图层；
...
![MVC-in-Rails-with-different-view][14]
...
除了上述框架中的 MVC 架构模式，还有一些其它的书籍或者资料对于 MVC 也有着不同的解释，比如维基百科的 [Model-view-controller][15] 条目，该条目是我们在 Google 搜索 [MVC][16] 时能够出现的前几个条目，这也是维基百科中的架构图能够出现在这篇文章中的原因 —— 有着广泛的受众。
...
> 上述内容出自 [Applications Programming in Smalltalk-80: How to use Model-View-Controller (MVC)][18] 一文。
...
![Essential-Dependencies-in-MVC][20]

> 虽然如上图所示，视图和控制器之间没有相互依赖，不过因为视图和控制器之间的依赖并不常用，所以图中将视图和控制器之间的依赖省略了。

#### 分离展示层

在 Martin Fowler 对于 Model-View-Controller 的描述中，MVC 最重要的概念就是分离展示层 [Separated Presentation][21]，如何在领域对象（Domain Object）和我们在屏幕上看到的 GUI 元素进行划分是 MVC 架构模式中最核心的问题。

GUI 应用程序由于其需要展示内容的特点，分为两个部分：一部分是用于展示内容的展示层（Presentation Layer），另一部分包含领域和数据逻辑的领域层（Domain Layer）。

![Presentation-Domain][22]
...
除了分离展示层，MVC 还与观察者同步 [Observer Synchronization][23] 关系紧密。因为在 MVC 模式中，模型可以单独工作，同时它对使用模型中数据的视图和控制器一无所知，为了保持模型的独立性，我们需要一种机制，当模型发生改变时，能够同时更新多个视图和控制器的内容；在这时，就需要以观察者同步的方式解决这个问题。

![Observer-Synchronization][24]
...
MVP 作为一个比较有争议的架构模式，在维基百科的 [Model-view-presenter][30] 词条中被描述为 MVC 设计模式的变种（derivation），自上个世纪 90 年代出现在 IBM 之后，随着不断的演化，虽然有着很多分支，不过 Martin Fowler 对 MVP 架构模式的定义最终被广泛接受和讨论。

![Standard-MVP][31]

在 MVP 中，Presenter 可以理解为松散的控制器，其中包含了视图的 UI 业务逻辑，所有从视图发出的事件，都会通过代理给 Presenter 进行处理；同时，Presenter 也通过视图暴露的接口与其进行通信。

目前常见的 MVP 架构模式其实都是它的变种：[Passive View][32] 和 [Supervising Controller][33]，接下来的内容也是围绕这两种变种进行展开的。
...
![Essential-Dependencies-in-Passive-Vie][35]
...
![Passive-View-with-Tags][36]

1. 当视图接收到来自用户的事件时，会将事件转交给 Presenter 进行处理；
1. 被动的视图向外界暴露接口，当需要更新视图时 Presenter 通过视图暴露的接口更新视图的内容；
1. Presenter 负责对模型进行操作和更新，在需要时取出其中存储的信息；
1. 当模型层改变时，可以将改变的信息发送给 **观察者** Presenter；
...
![Supervising-Controller-With-Tag][38]
...
> 上述内容取自 [What are MVP and MVC and what is the difference? · Stack Overflow][40] 中的 Model-View-Controller 部分。

## MVVM

相较于 MVC 和 MVP 模式，MVVM 在定义上就明确得多，同时，维基百科上对于 [Model-View-ViewModel][41] 的词条也没有歧义；不过，在谈 MVVM 架构模式之前，我们需要先了解它是如何发展和演变的。

### MVVM 的演变

早在 2004 年，Martin Fowler 发表了一篇名为 [Presentation Model][42] （以下简称为 PM 模式）的文章，PM 模式与 MVP 比较相似，它从视图层中分离了行为和状态；PM 模式中创建了一个视图的抽象，叫做 Presentation Model，而视图也成为了这个模型的『渲染』结果。

![PM-and-MVV][43]

2005 年，John Gossman 在他的博客上公布了 [Introduction to Model/View/ViewModel pattern for building WPF apps][44] 一文。MVVM 与 Martin Fowler 所说的 PM 模式其实是完全相同的，Fowler 提出的 PM 模式是一种与平台无关的创建视图抽象的方法，而 Gossman 的 MVVM 是专门用于 WPF 框架来简化用户界面的创建的模式；我们可以认为 **MVVM 是在 WPF 平台上对于 PM 模式的实现**。

> 有兴趣的读者可以阅读 [> Introduction to Model/View/ViewModel pattern for building WPF apps][45]>  获得更多与 MVVM 演化的相关信息。

### 展示模型

> 本节大部分内容都节选自 Martin Fowler 的 [Presentation Model][42] 一文。

既然 MVVM 是展示模型 [Presentation Model][42] 的一个实现，那么在介绍 Model-View-ViewModel 之前，我们就需要了解 PM 模式到底是什么。

在 MVC 一节中曾经有过对展示层和领域层进行分离的讨论，而 PM 模式就与分离展示层 [Separated Presentation][21] 有一定的关系。
...
* [MVC Index][50]
* [The Model-View-Controller (MVC) Its Past and Present][51]
* [The evolution of the Dolphin Smalltalk MVP application framework][52]
* [MVP: Model-View-Presenter · The Taligent Programming Model for C++ and Java][53]
* [Implementing the Model-View-ViewModel Pattern · MSDN][54]
* [GUI Architectures · Martin Fowler][55]
* [GUI 应用程序架构的十年变迁][56]
* [Elm Architecture Tutorial · GitHub][57]
* [Presentation Model · Martin Fowler][58]
* [Model-view-controller · Wikipedia][15]
* [Model-view-presenter · Wikipedia][30]
* [Model-view-viewmodel · Wikipedia][41]
* [Thing-Model-View-Editor][8]
* [ASP.NET MVC Overview · MSDN][9]
* [Intermediate Rails: Understanding Models, Views and Controllers][59]
* [Passive View · Martin Fowler][32]
* [Supervising Controller · Martin Fowler][33]
* [Applications Programming in Smalltalk-80: How to use Model-View-Controller (MVC)][18]
* [What are MVP and MVC and what is the difference? · Stack Overflow][40]
* [Model-View-Presenter Pattern][60]
* [Patterns - WPF Apps With The Model-View-ViewModel Design Pattern · MSDN][61]
* [Introduction to Model/View/ViewModel pattern for building WPF apps][45]
* [设计模式][62]

[0]: /tag/MVC
[1]: /tag/iOS
[2]: http://draveness.me/mvx-model.html
[3]: http://draveness.me/mvx-view.html
[4]: http://draveness.me/mvx-controller.html
[5]: http://draveness.me/mvx.html
[6]: ./img/MVC-MVVM-MVP.jpg
[7]: ./img/MVC-1979.jpg
[8]: http://heim.ifi.uio.no/%7Etrygver/1979/mvc-1/1979-05-MVC.pdf
[9]: https://msdn.microsoft.com/en-us/library/dd381412(v=vs.108).aspx
[10]: ./img/MVC-with-ASP.NET.jpg
[11]: ./img/MVC-with-Spring.jpg
[12]: ./img/MVC-with-iOS.jpg
[13]: ./img/MVC-with-Rails.jpg
[14]: ./img/MVC-%05in-Rails-with-different-view.jpg
[15]: https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller
[16]: https://www.google.com/search?q=MVC
[17]: ./img/MVC-in-Wikipedia.jpg
[18]: http://www.dgp.toronto.edu/%7Edwigdor/teaching/csc2524/2012_F/papers/mvc.pdf
[19]: ./img/Standard-MVC.jpg
[20]: ./img/Essential-Dependencies-in-MVC.jpg
[21]: https://www.martinfowler.com/eaaDev/SeparatedPresentation.html
[22]: ./img/Presentation-Domain.jpg
[23]: https://www.martinfowler.com/eaaDev/MediatedSynchronization.html
[24]: ./img/Observer-Synchronization.jpg
[25]: ./img/Main-Controller.jpg
[26]: ./img/Passive-Model.jpg
[27]: ./img/MVC-Web-App.jpg
[28]: ./img/MVC-App-Arch.jpg
[29]: ./img/MVC-MVC.jpg
[30]: https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93presenter
[31]: ./img/Standard-MVP.jpg
[32]: https://www.martinfowler.com/eaaDev/PassiveScreen.html
[33]: https://www.martinfowler.com/eaaDev/SupervisingPresenter.html
[34]: ./img/PassIve-View.jpg
[35]: ./img/Essential-Dependencies-in-Passive-View.jpg
[36]: ./img/Passive-View-with-Tags.jpg
[37]: ./img/Supervising-Controller.jpg
[38]: ./img/Supervising-Controller-With-Tag.jpg
[39]: ./img/Main-View-in-MVP.jpg
[40]: https://stackoverflow.com/questions/2056/what-are-mvp-and-mvc-and-what-is-the-difference
[41]: https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93viewmodel
[42]: https://www.martinfowler.com/eaaDev/PresentationModel.html
[43]: ./img/PM-and-MVVM.jpg
[44]: https://blogs.msdn.microsoft.com/johngossman/2005/10/08/introduction-to-modelviewviewmodel-pattern-for-building-wpf-apps/
[45]: https://blogs.msdn.microsoft.com/johngossman/2005/10/08/introduction-to-modelviewviewmodel-pattern-for-building-wpf-apps/%20%C2%B7%20MSDN
[46]: ./img/Presentation-Model.jpg
[47]: ./img/PM-View-Domain-Object.jpg
[48]: ./img/Model-View-ViewModel.jpg
[49]: ./img/Binder-View-ViewModel.jpg
[50]: http://heim.ifi.uio.no/%7Etrygver/themes/mvc/mvc-index.html
[51]: http://heim.ifi.uio.no/%7Etrygver/2003/javazone-jaoo/MVC_pattern.pdf
[52]: http://www.object-arts.com/downloads/papers/TwistingTheTriad.PDF
[53]: http://www.wildcrest.com/Potel/Portfolio/mvp.pdf
[54]: https://msdn.microsoft.com/en-us/library/ff798384.aspx
[55]: https://martinfowler.com/eaaDev/uiArchs.html
[56]: https://zhuanlan.zhihu.com/p/26799645
[57]: https://github.com/evancz/elm-architecture-tutorial/
[58]: https://martinfowler.com/eaaDev/PresentationModel.html
[59]: https://betterexplained.com/articles/intermediate-rails-understanding-models-views-and-controllers/
[60]: http://webclientguidance.codeplex.com/wikipage?title=ModelViewPresenterPatternDescription&referringTitle=MVPDocumentation
[61]: https://msdn.microsoft.com/en-us/magazine/dd419663.aspx
[62]: https://en.wikipedia.org/wiki/Design_Patterns