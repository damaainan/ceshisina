...
**1** 从下面的结果也能看出，MySQL 在索引 idx_fk_film_id 上使用了 ref 访问类型来执行 SQL。 **2** 删除索引后，访问类型变成了一个全表扫描（ ALL ），现在 MySQL 预估需要扫描 5073 条记录来完成这个查询。 Using where 表示 MySQL 将通过 WHERE 条件来筛选存储引擎返回的记录。 
