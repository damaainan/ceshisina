...
（一）基本特点：最基本、最简单、最常用的一种数据结构在这种结构中： 
...
a1称为线性表的第一个(首)结点，an称为线性表的最后一个(尾)结点。（三）线性表顺序存储 ：把线性表的结点按逻辑顺序依次存放在一组地址连续的存储单元里。用这种方法存储的线性表简称顺序表。 
