...
所有这些方案都不太好。在 User 类里面硬编码并没有解决实际问题，**`后续`**你依旧`无法在不修改 User 类代码的情况下实现更改会话名称的目的`。使用一个常量也是一个坏主意，因为 User 类现在依赖于这个常量来设置。将会话名称作为参数传递或者作为一组选项可能是最好的解决方案，但是仍然很糟糕，因为这种方式将与 User 类无关的数据与 User 类耦合在一起。
